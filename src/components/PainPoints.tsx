...
const ThinkingPerson = () => (
  <div className="relative flex flex-col items-center select-none">
    ...
    {["?", "?", "?"].map((q, i) => (
      <motion.span
        key={i}
        className="absolute font-black text-primary font-heebo"
    ...
    {/* Label */}
    <span className="font-heebo font-light text-xs text-foreground/40 mt-3 tracking-widest uppercase">מנהל / בעל עסק</span>
  </div>
);
    ...
    <section className="py-24 px-8 relative overflow-hidden bg-gradient-to-b from-background via-background to-surface-alt">
      ...
      <div className="max-w-6xl mx-auto" ref={ref}>
        ...
        >
          ...
          <h2 className="text-3xl md:text-5xl font-black text-foreground font-heebo leading-tight">
              ...
              {bubbles.map((bubble, i) => (
                ...
                >
                  <div className="glass-bento rounded-2xl rounded-tr-sm px-5 py-4 border border-white/50 shadow-sm flex items-start gap-3 text-right group hover:border-primary/30 hover:shadow-md transition-all duration-300 electric-glow-hover">
                    <p className="font-heebo font-light text-sm text-foreground/75 leading-relaxed flex-1">{bubble.text}</p>
                    <span className="text-lg mt-0.5 shrink-0">💭</span>
                  </div>
                  <div className="absolute -left-2 top-4 w-3 h-3 bg-[rgb(var(--glass-bg))] border-l border-b border-[rgb(var(--glass-border))] rotate-45 hidden md:block" />
                </motion.div>
              ))}
            </div>
          ...
          >
            ...
            >
              ...
              <div className="absolute inset-0 rounded-3xl bg-black/0 group-hover:bg-black/20 transition-all duration-300 flex items-center justify-center">
                <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-card/90 border border-border backdrop-blur-sm rounded-full px-4 py-2 flex items-center gap-2 shadow-lg">
                  <ZoomIn className="w-4 h-4 text-foreground" />
                  <span className="font-heebo font-light text-xs font-semibold text-foreground">לחץ להגדלה</span>
                </div>
              </div>
            </div>
          </motion.div>
        ...
        >
          ...
          <h3 className="text-2xl md:text-3xl font-black font-heebo text-foreground mb-3">
            אם אחת מהמחשבות האלה מוכרת —<br />
            <span className="gradient-text">אנחנו כבר חצי דרך לפתרון</span>
          </h3>
          <p className="font-heebo font-light text-sm text-foreground/60 mb-8 leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <motion.button
              ...
              className="bg-primary text-primary-foreground px-8 py-3.5 rounded-xl font-heebo font-semibold text-sm electric-glow hover:opacity-90 transition-all duration-200"
            >
              שיחת גילוי חינמית ← ← ←
            </motion.button>
            <motion.button
              ...
              className="glass-bento text-primary px-8 py-3.5 rounded-xl font-heebo font-semibold text-sm border border-primary/30 hover:border-primary/60 transition-all duration-200"
            >
              ראה את המסלולים שלי
            </motion.button>
          </div>
